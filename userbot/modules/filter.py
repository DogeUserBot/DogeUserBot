...
import re
from userbot import BOTLOG, BOTLOG_CHATID, CMD_HELP, BL_CHATS
...
async def filter_incoming_handler(handler):
    ...
    try:
        if not (await handler.get_sender()).bot:
            ...
            if handler.chat_id == -1001310554327:
                return
...
async def genelfilter(event):
    ...
    if event.chat_id == -1001310554327:
        await event.edit("`Kusura Bakma Ama Resmi Doge Grubunda Genel Filtre Eklenemez!`")
        return
    else:
...
async def add_new_filter(new_handler):
    ...
    if new_handler.chat_id == -1001310554327:
        await new_handler.edit("`Kusura Bakma Ama Resmi Doge Grubunda Filtre Eklenemez!`")
        return
    else:
