...
async def sleepybot(time):
    """ .sleep komutu Doge'nin birkaç saniye uyumasına olanak sağlar. """
...
CmdHelp('misc').add_command(
    'random', '<eşya1> <eşya2> ... <eşyaN>', 'Eşya listesinden rastgele bir eşya seçer', 'random doge uniborg userge'
).add_command(
    'sleep', '<süre>', 'Doge de bir insan, o da yoruluyor. Ara sıra biraz uyumasına izin ver.', 'sleep 30'
).add_command(
    'shutdown', None, 'Nostaljik bir şekilde botunuzu kapatın.'
).add_command(
    'repo', None, 'Doge botunun GitHub\'daki reposuna giden bir bağlantı.'
).add_command(
    'readme', None, 'Doge botunun GitHub\'daki README.md dosyasına giden bir bağlantı.'
).add_command(
