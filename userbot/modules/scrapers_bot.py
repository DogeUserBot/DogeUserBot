...
async def memeyap(event):
    ...
    if event.is_reply:
        ...
        if reply.photo:
            ...
            if os.path.exists("./DogeSticker.png"):
                os.remove("./DogeSticker.png")

            foto = await reply.download_media()
            im = Image.open(foto).convert("RGB")
            im.save("DogeSticker.png", "png")
            Resim = "DogeSticker.png"
        elif reply.sticker and reply.file.ext == ".tgs":
            sticker = await reply.download_media()
            os.system(f"lottie_convert.py --frame 0 -if lottie -of png '{sticker}' DogeSticker.png")
            os.remove(sticker)
            Resim = "DogeSticker.png"
        elif reply.media:
            Resim = await reply.download_media()
            Sure = os.system("ffmpeg -i '"+Resim+"' 2>&1 | grep Duration | awk '{print $2}' | tr -d , | awk -F ':' '{print ($3+$2*60+$1*3600)/2}'``")
            os.system(f"ffmpeg -i '{Resim}' -vcodec mjpeg -vframes 1 -an -f rawvideo -ss {Sure} DogeThumb.jpg")
            os.remove(Resim)
            Resim = 'DogeThumb.jpg'
        else:
            return await event.edit(LANG['REPLY_TO_MEME'])
            
        if os.path.exists("./bercmeme.png"):
            os.remove("./bercmeme.png")

        MemeYap(Resim, Text, font, Bottom, BottomText)
        await event.client.send_file(event.chat_id, "./bercmeme.png", reply_to=reply)
        await event.delete()
        os.remove(Resim)
    else:
...
CmdHelp('scrapers_bot').add_command(
    ...
    'meme', '<font> <üst;alt>', 'Fotoğrafa yazı ekleyin. İsterseniz font büyüklüğünü de yazabilirsiniz.', 'meme 30 doge;yigit'
).add_command(
