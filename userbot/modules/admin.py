...
"""
Grup yönetimi için yardımcı modül
"""
    ...
    if event.is_private:
        ...
        if not event.is_channel and event.is_group:
            for user_id in to_add_users.split(" "):
                await event.edit(f'**{user_id} kullanıcısını bu gruba ekliyorum...**')
                try:
                    ...
                    await event.edit(f'**{user_id} kullanıcısını gizlilik ayarları nedeniyle bu gruba ekleyemedim!**')
                    continue
                await event.edit(f'**{user_id} kullanıcısını bu gruba ekledim!**')
        else:
            for user_id in to_add_users.split(" "):
                await event.edit(f'**{user_id} kullanıcısını bu gruba ekliyorum...**')
                try:
...
async def tag(promt):
    """ .tag komutu ile belirlenen kişiyi kısıtlı yönetici yapar """
    # Hedef sohbeti almak
    chat = await promt.get_chat()
    # Yetkiyi sorgula
    admin = chat.admin_rights
...
async def sban(bon):
    """ .sban komutu belirlenen kişiyi gruptan sessizce yasaklar """
    # Yetki kontrolü
    chat = await bon.get_chat()
    admin = chat.admin_rights
    creator = chat.creator
    ...
    # Hedefi yasaklamak için kontrol et
    try:
        await bon.edit(LANG['BANNING'])
    except:
        await bon.reply(LANG['BANNING'])
...
async def sspider(spdr):
    """
    Bu fonksiyon sessizce susturmaya yarar.
    """
    # Fonksiyonun SQL modu altında çalışıp çalışmadığını kontrol et
    try:
        from userbot.modules.sql_helper.spam_mute_sql import mute
...
async def gspider(gspdr):
    ...
    if gmute(user.id) == False:
        ...
        if reason:
            ...
            await gspdr.edit({mention} LANG['GMUTED'].format(
                mention = f"[{user.first_name}](tg://user?id={user.id})",
            ))
...
async def get_users(show):
    ...
    try:
        ...
        await show.edit(
            "Hay aksi! Burası büyük bir grup. Gruptaki kullanıcıların listesini dosya olarak gönderiyorum.")
...
async def get_user_from_event(event):
    ...
    if event.reply_to_msg_id and not len(args) == 2:
        ...
        if not user:
            await event.edit("**Kişinin ya kullanıcı adını ya ID'sini ya da yanıtını iletin!**")
            return
...
async def unwarn(event):
    """ .unwarn kullanıcının uyarısını kaldırmaya yarar """
    # Yetki kontrolü
...
async def warn(event):
    """ .warn kullanıcıyı uyarmaya yarar """
    # Yetki kontrolü
    chat = await event.get_chat()
    admin = chat.admin_rights
    creator = chat.creator
...
async def Warn_Gmute(event, warn, user, reason = None):
    await event.delete()
    yeni = await event.reply(f"**Seni yeteri kadar uyarmıştım** [{user.first_name}](tg://user?id={user.id})**.\nSeni tamamen susturdum!\nHiçbir yerden mesaj gönderemeyeceksin.**")
    ...
    yeni2 = await yeni.reply("**Onu susturuyorum...**")
        
    if gmute(user.id) == False:
        await yeni2.edit(
            '**Onu zaten tamamen susturdum!\nO hiçbir yerden mesaj gönderemez.**')
    else:
        if reason != None:
            await yeni2.edit(f"**Kullanıcıyı tamamen susturdum!\nO hiçbir yerden mesaj gönderemez.\nNedeni:** __{reason}__")
        else:
            await yeni2.edit("**Kullanıcıyı tamamen susturdum!\nO hiçbir yerden mesaj gönderemez.**")
...
async def Warn_Gban(event, warn, user, reason = None):
    await event.delete()
    yeni = await event.reply(f"**Seni yeteri kadar uyarmıştım** [{user.first_name}](tg://user?id={user.id})**, her yerden yasakladım!**")
    ...
    yeni2 = await yeni.reply("**Yasaklıyorum...**")
        
    if gban(user.id) == False:
        await yeni2.edit(
            '**Onu her yerden zaten yasakladım.\nVe artık rahatsız edemez**')
    else:
        if reason != None:
            await yeni2.edit(f"**Kullanıcıyı her yerden yasakladım!\nO hiçbir yerden sahibimi rahatsız edemez.\nNedeni:** __{reason}__")
        else:
            await yeni2.edit("**Kullanıcıyı her yerden yasakladım!\nO hiçbir yerden sahibimi rahatsız edemez.**")
...
async def get_usersdel(show):
    """ .delusers komutu grup içinde ki silinen hesapları gösterir """
    info = await show.client.get_entity(show.chat_id)
    title = info.title if info.title else "this chat"
...
CmdHelp('admin').add_command(
        ...
        'tag', '<kullanıcı adı/yanıtlama>', 'Sohbetteki kişiyi kısıtlı yönetici yaparım.'
    ).add_command(
        'demote', '<kullanıcı adı/yanıtlama>', 'Sohbetteki kişinin yönetici izinlerini iptal eder.'
    ).add_command(
        'ban', '<kullanıcı adı/yanıtlama> <nedeni (isteğe bağlı)>', 'Gruptaki kişiyi yasaklar, sadece yöneticilerde çalışır.'
    ).add_command(
        'sban', '<kullanıcı adı/yanıtlama>', 'Gruptaki kişiyi sessizce yasaklar, sadece yöneticilerde çalışır.'
    ).add_command(
        ...
        'mute', '<kullanıcı adı/yanıtlama> <nedeni (isteğe bağlı)>', 'Gruptan belirttiğiniz kişiyi susturur.'
    ).add_command(
        'smute', '<kullanıcı adı/yanıtlama> <nedeni (isteğe bağlı)>', 'Gruptan belirttiğiniz kişiyi sessizce susturur.'
    ).add_command(
        ...
        'delusers', None, 'Grup içerisinde silinen hesapları gösterir.'
    ).add_command(
