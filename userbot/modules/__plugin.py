...
async def pport(event):
    ...
    if re.search(borg1, dosy):
        ...
        await event.client.send_file(event.chat_id, f"port_{dosya}", caption=LANG['DOGE_PLUGIN_CAPTION'])
        os.remove(f"port_{dosya}")
        os.remove(f"{dosya}")
    ...
    if re.search(r"@tgbot\.on\(.*pattern=(r|)\".*\".*\)", dosy):
        ...
        if (not type(Pattern) == list) or (len(Pattern) < 1 or len(Pattern[0]) < 1):
            if re.search(r'CmdHelp\(.*\)', dosy):
                ...
                return await event.edit(f'**Plugini kurdum!**\n__Bunun nasıl kullanıldığını öğrenebilmek için__ \n`.doge {cmdhelp}` __yaz.__')
            else:
                await reply_message.forward_to(PLUGIN_CHANNEL_ID)
                userbot.cmdhelp.CmdHelp(dosya).add_warning(':/\nBu pluginin komutlarını bulamadım!\Bunun nasıl çalıştığı hakkında hiçbir fikrim yok.').add()
        ...
        try: 
            if BOTLOG:
                await event.client.send_message(BOTLOG_CHATID, "#OTORESTART \n"
                                        "Plugin silindikten sonra yeniden başlatıldım.")
...
async def psend(event):
    ...
    if os.path.isfile(f"./userbot/modules/{modul}.py"):
        await event.client.send_file(event.chat_id, f"./userbot/modules/{modul}.py", caption=LANG['DOGE_PLUGIN_CAPTION'])
        await event.delete()
    else:
        await event.edit(LANG['NOT_FOUND_PLUGIN'])
...
async def ptest(event):
    ...
    return await event.edit(f'**Plugini geçici olarak kurdum!**\n__Hemen test edebilirsiniz!__\n**Yeniden başlatıldığımda bu plugin çalışmayacak.**')
